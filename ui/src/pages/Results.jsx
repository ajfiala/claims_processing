...
const DamageImage = ({ src, alt }) => {
  ...
  if (error || !imageUrl) {
    return (
      <div className="bg-gray-800 p-6 rounded border border-gray-700 text-gray-400 my-4 text-center">
  ...
  return (
    <div className="my-4 text-center">
      <img 
        ...
        onError={handleError}
        className={`max-w-full rounded-md border border-gray-700 shadow-md max-h-64 mx-auto ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
...
const markdownComponents = {
  h1: ({ node, ...props }) => <h1 className="text-2xl font-bold mb-2 border-b border-gray-700 pb-2" {...props} />,
  h2: ({ node, ...props }) => <h2 className="text-xl font-semibold mt-6 mb-3 border-b border-gray-700 pb-1" {...props} />,
...
const Results = () => {
  const [results, analyze, isThinking, scope, photos] = useStore(
    useShallow((state) => [state.results, state.analyze, state.isThinking, state.scope, state.photos])
  );
              ...
              <div className="flex flex-col items-center">
                {!isSuccess ? (
                  ...
                  <>
                    ...
                    <div className="bg-gray-900 p-6 rounded-md shadow-md border border-gray-700 max-w-3xl w-full mb-6">
                    ...
                    <button 
                      ...
                      onClick={() => { navigate("/" + scope?.policyId + "/upload") }}
                    >
                      Upload Another
                    </button>
                  </>
